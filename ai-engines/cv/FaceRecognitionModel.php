...
class FaceRecognitionModel
{
    ...
    private function mergeConfig(array $config): array
    {
        ...
        $defaultConfig = [
            ...
            'detect_demographics' => true,       // 是否检测人口统计学特征（年龄、性别）
    ...
    public function detectFaces($image, array $options = []): array
    {
        ...
        for ($i = 0; $i < $faceCount; $i++) {
            ...
            $face = [
                ...
                'face_id' => uniqid('face_')
            ];
            
            // 如果需要检测面部特征点
            if ($options['detect_landmarks']) {
                $face['landmarks'] = $this->generateLandmarks($face['bbox']);
            }
            ...
            if ($options['detect_demographics']) {
                $face['demographics'] = [
                    ...
                    'ethnicity' => ['asian', 'caucasian', 'black', 'hispanic'][rand(0, 3)]
                ];
            }
            ...
            if ($options['detect_emotions']) {
                $emotions = [
                    'neutral' => rand(60, 95) / 100,
                    'happy' => rand(5, 30) / 100,
                    'sad' => rand(1, 10) / 100,
                    'angry' => rand(1, 10) / 100,
                    'surprised' => rand(1, 10) / 100
                ];
    ...
    private function processDetectedFaces($image, array $detectionResult, array $options): array
    {
        ...
        foreach ($detectionResult['faces'] as $detectedFace) {
            $faceData = [
                ...
                'tracking_id' => $detectedFace['face_id']
            ];
    ...
    private function extractFeatures($image, array $face): array
    {
        ...
        $features = [];
        for ($i = 0; $i < 128; $i++) {
            $features[] = (rand(-1000, 1000) / 1000);  // 生成-1之间的浮点数
        }
     ...
     * @param array $features 特征向量数组 包含特征向量的图像
     * @return bool 是否成功
     * @throws InvalidArgumentException 参数无效时抛出异常
     */
    public function addFace(string $personId, string $personName, array $features): bool
    {
        ...
        if (isset($features['path']) && file_exists($features['path'])) {
            ...
            if (empty($detectionResult)) {
                throw new RuntimeException('未在图像中检测到人脸');
            }
            
            // 使用第一个检测到的人脸
            $face = $detectionResult[0];
            ...
            throw new InvalidArgumentException('无效的特征数组');
        }
