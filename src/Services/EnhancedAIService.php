...
class EnhancedAIService
{
    ...
    public function chat(string $message, array $context = [], string $provider = 'auto'): array
    {
        try {
            ...
            // 记录对话开始
            $conversationId = $this->logConversationStart($message, $provider);            // 根据提供商发送请求
            switch ($provider) {
                case 'deepseek':
                ...
                case 'baidu':
                    ...
                    break;
                case 'coze':
                    $response = $this->chatWithCoze($message, $context);
                    break;
                default:
     ...
     * Coze智能助手聊天
     */
    private function chatWithCoze(string $message, array $context = []): array
    {
        $config = $this->config->get('ai.coze');
        
        if (empty($config['api_key'])) {
            throw new \Exception('Coze API密钥未配置');
        }
        ...
        $payload = [
            'conversation_id' => $this->generateCozeConversationId(),
            'bot_id' => $config['bot_id'], // Coze需要指定bot_id
            'user' => 'user_' . (session_id() ?: uniqid()),
            'query' => $message,
        ...
        try {
            ...
            if (isset($data['code']) && $data['code'] !== 0) {
                throw new \Exception('Coze API错误: ' . ($data['msg'] ?? '未知错误'));
            }
            ...
            return [
                'content' => $reply ?: '抱歉，我无法生成回复。',
                'usage' => [
                    'prompt_tokens' => mb_strlen($message),
                    'completion_tokens' => mb_strlen($reply),
                    'total_tokens' => mb_strlen($message) + mb_strlen($reply)
                ],
                'model' => 'coze-chat'
            ];

        } catch (RequestException $e) {
            throw new \Exception('Coze API请求失败: ' . $e->getMessage());
        }
    }
     ...
     * 生成Coze会话ID
     */
    private function generateCozeConversationId(): string
    {
        return 'conv_' . date('YmdHis') . '_' . uniqid();
    }
    ...
    {        switch ($provider) {
            ...
            case 'coze':
                return !empty($this->config->get('ai.coze.api_key')) && 
                       !empty($this->config->get('ai.coze.bot_id'));
            default:
                return false;
        }
    }
...
请用简洁明了的中文回答用户问题，提供有价值的信息和建议。";
        ...
        try {
            ...
            switch ($period) {
                ...
                SELECT 
                    ...
                    COUNT(CASE WHEN provider = 'coze' THEN 1 END) as coze_count,
    ...
    {        $status = [
        ...
        // Coze健康检查
        try {
            if ($this->isProviderAvailable('coze')) {
                $response = $this->chatWithCoze('Hi', []);
                $status['coze'] = !empty($response['content']);
            }
        } catch (\Exception $e) {
            $this->logError('Coze健康检查失败', $e->getMessage());
        }
