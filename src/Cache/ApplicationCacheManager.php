...
class ApplicationCacheManager
{
     ...
     * 预热缓存（支持不同类型）
     */
    public function warmup(array $options = []): array
    {
        ...
        try {
            ...
            if ($type === 'all' || $type === 'file') {
                ...
                foreach ($criticalKeys as $key) {
                    if (!$this->fileCache->has($key)) {
                        $this->fileCache->set($key, "warmup_data_for_{$key}", 3600);
                        $warmedUp++;
                    }
                }
            }
            ...
            if ($type === 'all' || $type === 'memory') {
                ...
                foreach ($memoryKeys as $key) {
                    if (!isset($this->memoryCache[$key])) {
                        $this->setMemoryCache($key, "warmup_memory_data_for_{$key}", 1800);
                        $warmedUp++;
                    }
                }
            }
    ...
    public function analyzePerformance(): array
    {
        try {
            $stats = $this->getStats();
            $fileInfo = $this->getCacheFileInfo();
            
            // 计算命中率
            $totalRequests = $stats['requests']['total_requests'] ?? 0;
            $hitRate = 0;
            if ($totalRequests > 0) {
                $totalHits = ($stats['requests']['memory_hits'] ?? 0) + ($stats['requests']['file_hits'] ?? 0);
                $hitRate = round(($totalHits / $totalRequests) * 100, 2);
            }
            
            // 分析内存使用情况
            $memoryUsage = count($this->memoryCache);
            $memoryLimit = $this->config['memory_limit'] ?? 100;
            $memoryUtilization = round(($memoryUsage / $memoryLimit) * 100, 2);
            
            // 性能建议
            $recommendations = [];
            
            if ($hitRate < 50) {
                $recommendations[] = '命中率较低，建议优化缓存策略';
            }
            
            if ($memoryUtilization > 90) {
                $recommendations[] = '内存缓存使用率过高，建议增加内存限制';
            }
            
            if (($fileInfo['total_files'] ?? 0) > 1000) {
                $recommendations[] = '文件缓存数量过多，建议执行清理';
            }
            ...
            return [
                'performance_metrics' => [
                    ...
                    'file_count' => $fileInfo['total_files'] ?? 0,
                    'total_size' => $fileInfo['total_size'] ?? 0,
    ...
    public function setCacheConfig(array $config): array
    {
        try {
            ...
            // 更新配置
            $this->config = array_merge($this->config, $validConfig);
            
            return [
                'success' => true,
                'updated_config' => $validConfig,
                'current_config' => $this->getCacheConfig(),
                'timestamp' => date('Y-m-d H:i:s')
            ];
