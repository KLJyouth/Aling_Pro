...
class PerformanceMonitor {
    ...
    private function collectApiMetrics() {
        ...
        $apiStats = $this->monitoring->getPerformanceStats(1); // 最近1小时
        ...
        if ($apiStats['total_requests'] > 0) {
            ...
            echo "   ✅ 最大响应时间: " . round($apiStats['max_response_time'] * 1000, 2) . "ms\n";
            echo "   ✅ 错误率: " . round($apiStats['error_rate'], 2) . "%\n";
        } else {
            echo "   ⚠️ 暂无API请求数据\n";
        }
        echo "\n";
    ...
    private function analyzeMetrics() {
        ...
        if (isset($this->metrics['api']['avg_response_time'])) {
            ...
            if ($avgResponseTime > $this->thresholds['response_time']) {
                ...
                $warnings[] = "API响应时间接近阈值: {$avgResponseTime}ms";
            }
            
            if ($this->metrics['api']['error_rate'] > $this->thresholds['error_rate']) {
                $issues[] = "API错误率过高: " . $this->metrics['api']['error_rate'] . "%";
            }
        }
        ...
        if ($this->metrics['resources']['disk_usage'] > $this->thresholds['disk_usage']) {
            $issues[] = "磁盘使用率过高: " . round($this->metrics['resources']['disk_usage'], 2) . "%";
        }
    ...
    private function displayDashboard() {
        ...
        if (isset($this->metrics['api']['total_requests']) && $this->metrics['api']['total_requests'] > 0) {
            ...
            echo "   错误率: " . round($this->metrics['api']['error_rate'], 2) . "%\n";
        } else {
            echo "   暂无API性能数据\n";
        }
        echo "\n";
    ...
    private function generateRecommendations() {
        ...
        if ($memoryUsage > $memoryLimit * 0.8) {
            ...
            $recommendations[] = "启用OPcache以减少内存使用";
            $recommendations[] = "优化数据库查询以减少内存消耗";
        }
        
        // 磁盘空间建议
        if ($this->metrics['resources']['disk_usage'] > 80) {
            $recommendations[] = "清理日志文件和临时文件";
        ...
        // 通用建议
        $recommendations[] = "定期运行系统优化器";
        $recommendations[] = "监控系统日志以及时发现问题";
    ...
    public function startRealTimeMonitoring($interval = 30) {
        echo "🔄 启动实时监控模式 (每{$interval}秒更新)\n";
        echo "按 Ctrl+C 停止监控\n\n";
        
        while (true) {
            system('clear'); // 在Windows上使用 'cls'
    ...
    private function formatUptime($seconds) {
        ...
        if ($seconds > 0) $parts[] = round($seconds, 1) . "秒";
