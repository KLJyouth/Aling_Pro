...
class SecurityScanner
{
    ...
    private function checkEnvironmentSecurity()
    {
        ...
        if (file_exists('.env')) {
            ...
            if ($perms & 0x0004) {  // 其他用户可读
                $this->addError("ENV_PERMS", ".env 文件对其他用户可读");
            } else {
        ...
        if ($debug === 'true') {
            ...
            $this->addResult("DEBUG_MODE", "调试模式已关闭");
        }
        ...
        if ($forceHttps !== 'true') {
            ...
            $this->addResult("HTTPS_FORCE", "HTTPS 强制已启用");
        }
        
        // 检查会话安全
        $secureSession = $_ENV['SESSION_SECURE_COOKIE'] ?? 'false';
        if ($secureSession !== 'true') {
            $this->addWarning("SESSION_SECURITY", "会话 Cookie 应启用 Secure 标志");
        } else {
    ...
    private function checkDatabaseSecurity()
    {
        ...
        try {
            ...
            $this->addResult("DB_CONNECTION", "数据库连接正常");
            
            // 检查用户权限
            if ($username === 'root') {
                $this->addWarning("DB_USER_PRIVILEGE", "建议使用专用数据库用户而非 root");
            } else {
                $this->addResult("DB_USER_PRIVILEGE", "使用专用数据库用户");
            }
            ...
            $this->addResult("DB_VERSION", "数据库版本: " . $version);
            
        } catch (Exception $e) {
            $this->addError("DB_CONNECTION", "数据库连接失败: " . $e->getMessage());
        }
    ...
    private function checkFileSecurity()
    {
        ...
        foreach ($sensitiveFiles as $file => $desc) {
            if (file_exists($file)) {
                ...
                if ($webAccessible) {
                    ...
                    $this->addResult("FILE_PROTECTION", "{$desc} 受保护");
                }
            }
        }
        ...
        if (is_dir($uploadPath)) {
            ...
            if ($perms & 0x0040) {  // 检查执行权限
                $this->addWarning("UPLOAD_PERMS", "上传目录不应有执行权限");
            } else {
    ...
    private function checkNetworkSecurity()
    {
        ...
        if (empty($redisPassword)) {
            ...
            $this->addResult("REDIS_AUTH", "Redis 已配置密码保护");
        }
        ...
        if ($redisPort === '6379') {
            ...
            $this->addResult("REDIS_PORT", "使用非默认 Redis 端口");
        }
        
        // 检查 WebSocket 安全
        $wsSSL = $_ENV['WEBSOCKET_SSL'] ?? 'false';
        if ($wsSSL !== 'true') {
            $this->addWarning("WS_SSL", "WebSocket 应启用 SSL");
        } else {
            $this->addResult("WS_SSL", "WebSocket SSL 已启用");
        }
    ...
    private function generateSecurityReport()
    {
        echo "📊 生成安全扫描报告...\n";
        echo str_repeat("=", 60) . "\n";
        echo "🛡️  AlingAi Pro 安全扫描报告\n";
        echo str_repeat("=", 60) . "\n";
        echo "扫描时间: " . date('Y-m-d H:i:s') . "\n\n";
        
        echo "📈 扫描统计:\n";
        echo "  ✅ 通过检查: " . count($this->results) . "\n";
