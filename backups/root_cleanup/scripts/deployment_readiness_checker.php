...
class DeploymentReadinessChecker {
    ...
    private function initializeReport() {
        ...
        echo "║              🚀 部署就绪检查工具                            ║\n";
        echo "╠══════════════════════════════════════════════════════════════╣\n";
        echo "║  检查时间: " . date('Y-m-d H:i:s') . str_repeat(' ', 25) . "║\n";
            ...
            'file_permissions' => function() {
                ...
                foreach ($sensitiveFiles as $file) {
                    ...
                    if (file_exists($path)) {
                        ...
                        // 在Windows上权限检查不同，这里简化处理
                        if (DIRECTORY_SEPARATOR === '\\') {
                            echo "      ✅ Windows环境下文件权限: $file\n";
                        } else {
        ...
        foreach ($securityChecks as $checkName => $checkFunc) {
            ...
            if (!$result) {
                if ($checkName === 'secret_keys') {
                    $this->criticalIssues[] = "安全检查失败: $checkName";
                } else {
    ...
    private function generateReadinessReport() {
        ...
        echo "║  部署状态: " . $this->getStatusText($deploymentStatus) . str_repeat(' ', 30 - mb_strlen($this->getStatusText($deploymentStatus), 'UTF-8')) . "║\n";
        echo "║  关键问题: " . count($this->criticalIssues) . " 个                                            ║\n";
        echo "║  警告信息: " . count($this->warnings) . " 个                                            ║\n";
        echo "║  建议项目: " . count($this->recommendations) . " 个                                            ║\n";
        echo "╚══════════════════════════════════════════════════════════════╝\n\n";
...
// 执行部署就绪检查
echo "正在启动 AlingAi Pro 5.0 部署就绪检查...\n\n";
