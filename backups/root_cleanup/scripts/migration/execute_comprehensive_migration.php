...
class ComprehensiveMigrationExecutor
{
    ...
    {
        // 特殊处理：某些目录已经在public中存在
        if (is_dir($targetDir)) {
            echo "    ⚠️  目标目录已存在，检查是否需要合并: {$dirName}/\n";
            
            if ($dirName === 'docs') {
                // docs目录已经部分迁移，只需要移动剩余内容
    ...
    private function copySelectiveFiles(string $source, string $target, array $extensions, array $patterns): void
    {
        ...
        foreach ($iterator as $item) {
            if ($item->isFile()) {
                ...
                $shouldCopy = in_array($extension, $extensions);
...
HTACCESS;
    ...
    private function createDirectorySecurityConfigs(): void
    {
        $securityConfigs = [
            'install' => [
                'type' => 'ip_restrict',
                'note' => '安装工具，建议完成后移除或限制访问'
            ],
            'test' => [
                'type' => 'ip_restrict', 
                'note' => '测试工具，生产环境建议限制访问'
            ],
            'docs' => [
                'type' => 'allow_all',
                'note' => '文档目录，允许公开访问'
    ...
    private function generateReport(): void
    {
        ...
        echo "  ✅ 迁移报告已保存: " . basename($reportPath) . "\n";
        echo "\n🎉 综合迁移执行完成！\n\n";
...
### 高优先级完全迁移
已迁移目录: {$highPriorityList}

### 部分选择性迁移  
已迁移目录: {$partialMigrationList}
