...
class FolderMerger 
{
    ...
    public function findDuplicateFolders() {
        ...
        echo "Public文件夹: " . count($publicFolders) . " 个\n";
        echo "重名文件夹: " . count($this->duplicateFolders) . " 个\n\n";
        ...
        if (!is_dir($targetPath)) {
            ...
            if (!$dryRun) {
                if ($this->moveDirectory($sourcePath, $targetPath)) {
                    ...
                    echo "   ❌ 移动失败\n";
                    $this->errors[] = "移动文件夹失败: $folderName";
                }
            } else {
                echo "   🔍 [预览] 将移动整个文件夹\n";
            }
    ...
    public function deleteOriginalFolders($dryRun = false) {
        ...
        echo ($dryRun ? "🔍 预览模式" : "🗑️ 执行删除") . " - 删除原始重名文件夹...\n";
        echo "----------------------------------------\n";
        ...
        foreach ($this->duplicateFolders as $folderName) {
            ...
            if (!$dryRun) {
                if ($this->deleteDirectory($folderPath)) {
                    ...
                    $this->errors[] = "删除文件夹失败: $folderName";
                }
            } else {
                echo "🔍 [预览] 将删除: $folderName\n";
            }
        }
    ...
    public function generateReport() {
        ...
        echo "📊 统计信息:\n";
        echo "   🔍 发现重名文件夹: " . count($this->duplicateFolders) . " 个\n";
        echo "   ✅ 成功合并文件夹: " . count($this->mergedFolders) . " 个\n";
        echo "   ⚠️ 跳过的文件: " . count($this->skippedFiles) . " 个\n";
