...
class SystemOptimizer {
    ...
    public function runAllOptimizations() {
        ...
        foreach ($this->optimizations as $key => $config) {
            ...
            echo "🔧 执行: {$config['name']} (优先级: {$config['priority']})\n";
            echo str_repeat("-", 40) . "\n";
    ...
    private function runOptimization($type) {
        try {
            switch ($type) {
                ...
                default:
                    return ['success' => false, 'message' => '未知的优化任务'];
            }
        } catch (Exception $e) {
    ...
    private function optimizeComposer() {
        ...
        return [
            'success' => $success,
            'message' => $success ? 'Composer自动加载已优化' : 'Composer优化失败',
            'details' => $output
        ];
    }
    ...
    private function warmupCache() {
        ...
        return [
            ...
            'message' => "预热了 {$warmedUp} 个端点，{$failed} 个失败",
    ...
    private function cleanupLogs() {
        ...
        return [
            'success' => true,
            'message' => "清理了 {$cleaned} 个日志文件，释放 " . $this->formatBytes($totalSize),
    ...
    private function securityScan() {
        ...
        return [
            ...
            'message' => count($issues) === 0 ? '未发现安全问题' : '发现 ' . count($issues) . ' 个安全问题',
            'details' => $issues
        ];
    }
