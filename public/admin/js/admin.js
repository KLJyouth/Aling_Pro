/**
 * AlingAI Pro 5.1 管理后台 JavaScript
 * @version 2.1.0
 * @author AlingAi Team
 */
 ...
 * 更新安全检查结果
 */
function updateSecurityCheckResults(data) {
    // 更新整体状态
    const statusElement = document.getElementById('security-overall-status');
    if (statusElement) {
        let statusClass = 'text-green-500';
        let statusText = '安全';
        
        if (data.overall_status === 'vulnerable') {
    ...
    // 更新漏洞计数
    updateElement('vulnerabilities-count', data.vulnerabilities_found);
    updateElement('critical-issues-count', data.critical_issues);
    
    // 更新检查结果列表
    const checksListElement = document.getElementById('security-checks-list');
    if (checksListElement) {
        checksListElement.innerHTML = '';
        
        for (const [checkName, checkResult] of Object.entries(data.checks)) {
            const checkItem = document.createElement('div');
            checkItem.className = 'border-b border-gray-200 py-3';
            
            let statusIcon = '<i class="fas fa-check-circle text-green-500 mr-2"></i>';
            if (checkResult.status === 'vulnerable') {
                statusIcon = '<i class="fas fa-exclamation-triangle text-yellow-500 mr-2"></i>';
                if (checkResult.severity === 'critical') {
                    statusIcon = '<i class="fas fa-times-circle text-red-500 mr-2"></i>';
                }
            }
            
            const displayName = getSecurityCheckDisplayName(checkName);
            ...
            if (checkResult.recommendation) {
                recommendationHtml = `
                    <div class="mt-2 text-sm font-medium text-blue-600">
                        <i class="fas fa-lightbulb mr-1"></i> 建议: ${checkResult.recommendation}
                    </div>
                `;
            }
            ...
            checkItem.innerHTML = `
                <div class="flex items-center justify-between">
                    ...
                    <span class="text-sm text-gray-500">${checkResult.status === 'secure' ? '安全' : '需要注意'}</span>
                </div>
                ${issuesHtml}
                ${recommendationHtml}
            `;
            
            checksListElement.appendChild(checkItem);
        }
    }
    ...
    if (recommendationsElement) {
        ...
        if (data.recommendations && data.recommendations.length > 0) {
            ...
            recommendationsList.className = 'list-disc pl-5';
            ...
            recommendationsElement.appendChild(recommendationsList);
        } else {
            recommendationsElement.textContent = '没有安全建议，系统安全状态良好。';
        }
    }
    
    // 更新安全分数可视化
    updateSecurityScoreVisualization(data);
}

/**
 * 获取安全检查项显示名称
 */
function getSecurityCheckDisplayName(checkName) {
    const displayNames = {
        'file_permissions': '文件权限',
        'config_security': '配置安全',
        'dependencies': '依赖项安全',
        'api_security': 'API安全',
        'database_security': '数据库安全',
 ...
 * 更新安全分数可视化
 */
function updateSecurityScoreVisualization(data) {
    const securityScoreElement = document.getElementById('security-score-chart');
    if (!securityScoreElement) return;
    
    // 计算安全分数
    let securityScore = 100;
    securityScore -= data.vulnerabilities_found * 5;
    securityScore -= data.critical_issues * 15;
    securityScore = Math.max(0, Math.min(100, securityScore));
    
    // 如果已有图表，更新它
    if (charts.securityScore) {
        charts.securityScore.data.datasets[0].data = [securityScore, 100 - securityScore];
        charts.securityScore.update();
        return;
    }
    
    // 创建新图表
    const ctx = securityScoreElement.getContext('2d');
    charts.securityScore = new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [{
                ...
                backgroundColor: [
                    securityScore > 80 ? '#10B981' : securityScore > 60 ? '#F59E0B' : '#EF4444',
                    '#E5E7EB'
                ],
                borderWidth: 0
            }]
    ...
    if (scoreText) {
        scoreText.textContent = securityScore;
    }
}
