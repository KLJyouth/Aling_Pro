...
runTest('枚举类型', function() {
    if (!class_exists('BackedEnum')) {
        return "PHP 8.1 枚举类型不可用，但这不影响现有代�?;
    }
...
runTest('array_is_list 函数', function() {
    if (!function_exists('array_is_list')) {
        return "array_is_list 函数不可用，但这不影响现有代�?;
    }
...
// 2. 测试数据库连�?
runTest('数据库连�?, function() {
    if (!file_exists(__DIR__ . '/src/Config/config.php')) {
        return "找不到配置文件，跳过数据库测�?;
    }
    
    // 尝试加载配置
    $config = include __DIR__ . '/src/Config/config.php';
    
    if (!isset($config['database']) || !is_[$config['database'])) {
        return "配置文件中没有数据库配置，跳过测�?;
    }
    ...
    try {
        ...
        return "数据库连接失�? " . $e->getMessage(];
    }
}];

// 3. 测试核心类加�?
runTest('核心类加�?, function() {
    $coreClasses = [
