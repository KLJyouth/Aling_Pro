...
try {
    ...
    $sm2 = new class($logger) {
        ...
        public function generateKeyPair(): array {
            return [
                'public_key' => random_bytes(64],  // 64字节二进制公�?
                'private_key' => random_bytes(32], // 32字节二进制私�?
                'algorithm' => 'SM2',
                'curve' => 'sm2p256v1',
                'key_size' => 256,
                'created_at' => time()
    ...
    // 准备测试数据
    $originalData = "AlingAi Pro 6.0 - 企业级AI平台量子安全数据：用户登录信息、交易记录、AI模型参数等核心机密数�?;
    ...
    // 步骤1: 量子密钥分发 (模拟QKD生成的密�?
    echo "🔑 步骤1: 量子密钥分发 (QKD)\n";
    $K1 = hash('sha256', 'QKD-BB84-' . microtime(true) . '-quantum-secure', true];
    $K1 = substr($K1, 0, 16]; // SM4需�?28位密�?
    echo "   QKD协议: BB84\n";
    echo "   生成密钥: " . bin2hex($K1) . "\n";
    echo "   密钥长度: " . (strlen($K1) * 8) . "位\n";
    echo "   �?QKD密钥生成完成\n\n";    // 步骤2: SM4对称加密
    echo "🔐 步骤2: SM4对称加密\n";
    $encryptResult = $sm4->encrypt($originalData, $K1]; // 直接使用二进制密�?
    if (!is_[$encryptResult) || !isset($encryptResult['ciphertext'])) {
        throw new Exception('SM4加密返回格式错误'];
    }
    $encryptedData = $encryptResult['ciphertext']; // 获取密文字符�?
    echo "   算法: SM4-128位\n";
    ...
    echo "   系统就绪状�? " . ($allSuccess ? "�?生产就绪" : "⚠️ 需要修�?) . "\n";
    echo "   安全等级: 量子级安全保护\n";
