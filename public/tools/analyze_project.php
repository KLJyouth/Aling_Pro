...
function generatePriorityReport($stats)
{
    ...
    $report .= "- 基本结构文件�? " . $stats['basic_structure_files'] . " (" . round(($stats['basic_structure_files'] / $stats['total_files']) * 100, 2) . "%)\n";
    $report .= "- 完整文件�? " . $stats['complete_files'] . " (" . round(($stats['complete_files'] / $stats['total_files']) * 100, 2) . "%)\n";
    $report .= "- 测试文件�? " . $stats['test_files'] . "\n";
    $report .= "- 测试覆盖�? " . round($stats['test_coverage'],  2) . "%\n\n";
    
    $report .= "## 目录优先级\n\n";
    $report .= "| 目录 | 总文件数 | 空文�?| 基本结构 | 完整文件 | 优先级得�?|\n";
    $report .= "|------|----------|--------|----------|----------|------------|\n";
    ...
    foreach ($topPriorities as $dirName => $dirStats) {
        $report .= "1. **{$dirName}** - 优先级得�? " . round($dirStats['priority_score'],  2) . "\n";
        $report .= "   - 空文�? {$dirStats['empty_files']}, 基本结构: {$dirStats['basic_structure_files']}, 完整文件: {$dirStats['complete_files']}\n";
    }
...
echo "- 基本结构文件�? " . $stats['basic_structure_files'] . "\n";
echo "- 完整文件�? " . $stats['complete_files'] . "\n";
echo "- 测试文件�? " . $stats['test_files'] . "\n";
echo "- 测试覆盖�? " . round($stats['test_coverage'],  2) . "%\n";
