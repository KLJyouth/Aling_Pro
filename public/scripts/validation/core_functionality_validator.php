...
class CoreFunctionalityValidator {
    ...
    private $serverPort = 8001; // 使用不同端口避免冲突
      public function __construct() {
        $this->rootDir = dirname(dirname(__DIR__)]; // 回到项目根目�?
    ...
    private function startTestServer() {
        ...
        if (@file_get_contents($testUrl, false, $context)) {
            echo "�?测试服务器启动成�? http://localhost:{$this->serverPort}\n";
    ...
    private function getTestName($test) {
        $names = [
            'php_version' => 'PHP版本兼容�?,
            'php_extensions' => 'PHP扩展完整�?,
            'file_structure' => '文件结构完整�?,
            'dependencies' => '依赖包加�?,
            'server_start' => '服务器启�?,
            'web_access' => 'Web页面访问',
            'api_endpoints' => 'API端点响应',
            'static_resources' => '静态资源访�?
        ];
