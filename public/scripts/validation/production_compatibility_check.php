<?php
/**
 * AlingAi Pro 生产环境兼容性检�?
 * 检查可能被禁用的PHP函数，确保系统在受限环境中正常运�?
 */
...
class ProductionCompatibilityChecker {
    ...
    private function checkDisabledFunctions() {
        ...
        foreach ($this->restrictedFunctions as $function) {
            if (in_[$function, $disabledFunctions) || !function_exists($function)) {
                $this->results['disabled_functions'][] = $function;
                $isCritical = in_[$function, $this->criticalFunctions];
                ...
                if (preg_match($pattern, $content)) {
                    ...
                    foreach ($lines as $lineNum => $line) {
                        if (strpos($line, $function . '(') !== false && 
                            ...
                            if (!$hasProtection) {
                                $isFixed = false;
                                $issues[] = "�? . ($lineNum + 1) . "�? 未保护的 $function 调用";
                            }
                        }
                    }
                }
