...
class PathDependencyAnalyzer {
    ...
    private function checkAutoloadPaths() {
        ...
        if (!file_exists($vendorAutoload)) {
            $this->issues[] = "�?Vendor autoload文件不存�? $vendorAutoload";
    ...
    private function checkIndexPhpPaths() {
        ...
        if (!file_exists($indexPath)) {
            $this->issues[] = "�?index.php文件不存�? $indexPath";
            echo "�?index.php文件不存在\n";
            return;
        }
        ...
        if (strpos($content, "define('APP_ROOT', dirname(__DIR__))") !== false) {
            ...
            $this->warnings[] = "⚠️ APP_ROOT定义可能有问�?;
        ...
        if (strpos($content, "APP_ROOT . '/vendor/autoload.php'") !== false) {
            ...
            $this->warnings[] = "⚠️ Vendor autoload路径可能有问�?;
    ...
    private function checkCoreDependencies() {
        ...
        foreach ($coreFiles as $file => $description) {
            ...
            if (file_exists($fullPath)) {
                ...
                $this->warnings[] = "⚠️ $description 文件不存�? $file";
                echo "⚠️ $description 文件不存�? $file\n";
            }
        }
        echo "\n";
    }
    ...
    private function checkVendorPaths() {
        ...
        if (!is_dir($vendorDir)) {
            $this->issues[] = "�?Vendor目录不存在，需要运�?composer install";
            echo "�?Vendor目录不存在，需要运�?composer install\n";
        } else {
            echo "�?Vendor目录存在\n";
        ...
        if (!is_dir($srcDir)) {
            ...
            foreach ($coreDirectories as $dir => $description) {
                ...
                if (is_dir($fullPath)) {
                    ...
                    $this->warnings[] = "⚠️ $description 目录不存�? src/$dir";
                    echo "⚠️ $description 目录不存�? src/$dir\n";
                }
            }
        }
        echo "\n";
    ...
    private function generateSuggestions() {
        if (!empty($this->issues)) {
            foreach ($this->issues as $issue) {
                if (strpos($issue, 'Vendor') !== false) {
                    $this->suggestions[] = "🔧 运行 'composer install' 安装依赖�?;
                }
                if (strpos($issue, '核心应用�?) !== false) {
                    $this->suggestions[] = "🔧 检�?src/Core/AlingAiProApplication.php 文件是否存在";
        ...
        if (!empty($this->warnings)) {
            ...
            $this->suggestions[] = "🔄 确保所有路径使用相对于APP_ROOT的正确引�?;
        }
        
        // 通用建议
        $this->suggestions[] = "🧪 使用 php -t public/ -S localhost:8000 测试服务器运�?;
        $this->suggestions[] = "📋 检查PHP错误日志定位具体问题";
        $this->suggestions[] = "🔒 确保.env文件在根目录且配置正�?;
    }
}
