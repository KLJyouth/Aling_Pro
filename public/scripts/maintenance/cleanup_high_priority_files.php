...
class HighPriorityFilesCleanup
{
    ...
    {
        $this->cleanupFileCategories($this->highPriorityFiles, '高优先级'];
        $this->cleanupFileCategories($this->mediumPriorityFiles, '中等优先�?];
    }
    ...
    private function generateReport(): void
    {
        ...
        if (!empty($this->backedUpFiles)) {
            $report .= "## 📦 备份文件位置\n\n";
            $report .= "所有删除的文件都已备份�? `{$this->backupDir}`\n\n";
        ...
        $report .= "3. **清理测试**: 考虑将测试文件移动到专门�?`tests/` 目录\n";
        ...
        echo "  - 健康检�? http://localhost:8000/monitor/health.php\n";
    }
    ...
    private function getCategoryName(string $category): string
    {
        $names = [
            ...
            'old_websocket' => '多余WebSocket服务�?,
            'old_migrations' => '已完成迁移脚�?,
            'temp_system' => '临时系统文件',
        ];
