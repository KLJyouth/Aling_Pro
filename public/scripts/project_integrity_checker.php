...
class ProjectIntegrityChecker {
    ...
    private function checkDependencies() {
        ...
        if (file_exists($this->rootPath . '/vendor/autoload.php')) {
            ...
            $this->criticalIssues[] = "Composer依赖未安�?;
            echo "   �?Composer依赖未安装\n";
        }
            ...
            try {
                ...
                if (isset($dbConfig['connections']['file'])) {
                    ...
                    if (is_dir($fileDbPath)) {
                        ...
                        $this->warnings[] = "文件数据库目录不存在，建议运�?init_file_database.php";
                        echo "   ⚠️ 文件数据库目录不存在\n";
                    }
                }
                // 检查MySQL数据库（作为备用�?
                elseif (isset($dbConfig['connections']['mysql'])) {
                    ...
                    if (count($tables) > 0) {
                        ...
                        $this->warnings[] = "数据库中没有表，可能需要运行迁�?;
                        echo "   ⚠️ 数据库为空\n";
                    }
                } else {
                    $this->criticalIssues[] = "数据库配置格式错�?;
                    echo "   �?数据库配置格式错误\n";
                }
                
            } catch (PDOException $e) {
                $this->warnings[] = "MySQL数据库连接失败，使用文件数据�? " . $e->getMessage(];
                ...
                $this->warnings[] = "数据库检查失�? " . $e->getMessage(];
                echo "   ⚠️ 数据库检查失败\n";
            }
        } else {
            ...
            if (is_[$results)) {
                foreach ($results as $key => $value) {
                    ...
                    if (is_bool($value) && $value) {
                        $passedChecks++;
                    } elseif (is_[$value) && !empty($value) && !in_[$key, ['missing_directories', 'missing_files', 'invalid_configs', 'missing_configs', 'missing_extensions', 'non_writable_directories', 'non_executable_files'])) {
                        $passedChecks++;
                    }
                }
            }
    ...
    private function displaySummary() {
        ...
        echo "�?                   📊 完整性检查摘�?                       ║\n";
        echo "╠══════════════════════════════════════════════════════════════╣\n";
        echo "�? 整体健康分数: {$overallHealth}%  状�? {$healthStatus}" . str_repeat(" ", 28 - mb_strlen($healthStatus)) . "║\n";
        echo "�? 关键问题: " . count($this->criticalIssues) . " �? . str_repeat(" ", 47 - strlen(count($this->criticalIssues))) . "║\n";
        echo "�? 警告信息: " . count($this->warnings) . " �? . str_repeat(" ", 47 - strlen(count($this->warnings))) . "║\n";
        echo "╚══════════════════════════════════════════════════════════════╝\n";
