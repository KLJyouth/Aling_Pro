...
class ComprehensiveDirectoryScanner
{
    ...
    private function getRootDirectories(): array
    {
        ...
        foreach ($items as $item) {
            ...
            if (is_dir($fullPath) && !in_[$item, ['.git', '.vscode', 'node_modules', 'vendor'])) {
                $directories[] = $item;
            }
        }
    ...
    private function categorizeFile(string $filename, string $filePath, array &$analysis): void
    {
        ...
        // 检查是否为Web可访问文�?
        if (in_[$extension, $this->webAccessiblePatterns['extensions'])) {
            $analysis['web_accessible_files'][] = $filename;
            $analysis['public_migration_score'] += 10;
        }
        
        // 检查静态资�?
        if (in_[$extension, ['css', 'js', 'png', 'jpg', 'jpeg', 'gif', 'svg', 'ico', 'woff', 'woff2', 'ttf'])) {
            $analysis['static_assets'][] = $filename;
            $analysis['public_migration_score'] += 15;
            $analysis['reasons'][] = "静态资源文�? {$filename}";
        }
        ...
        // 检查前端文�?
        if (in_[$filename, $this->webAccessiblePatterns['frontend_files'])) {
    ...
    private function calculateMigrationScore(array &$analysis): void
    {
        ...
        foreach ($privateDirectoryNames as $name) {
            if (strpos($dirName, $name) !== false) {
                ...
                $analysis['reasons'][] = "目录名称表明为私有目�? {$name}";
                break;
            }
        }
    ...
    private function analyzeForPublicMigration(): void
    {
        ...
        echo "\n📈 迁移统计:\n";
        echo "  �?完全迁移候�? " . count($migrationCandidates) . " 个目录\n";
        echo "  �?部分迁移候�? " . count($partialMigrationCandidates) . " 个目录\n";
    ...
    private function generateFullMigrationPlan(string $dirName, array $analysis): void
    {
        ...
        echo "  目标位置: {$targetDir}/\n";
        echo "  迁移文件�? " . count($analysis['files']) . "\n";
        
        if (!empty($analysis['static_assets'])) {
            echo "  静态资�?(" . count($analysis['static_assets']) . "):\n";
...
MARKDOWN;
        ...
        foreach ($this->analysisResults as $dirName => $analysis) {
            ...
            if (!empty($analysis['static_assets'])) {
                $content .= "- **静态资�?*: " . count($analysis['static_assets']) . " 个\n";
            }
...
1. **配置文件保护**: 确保所�?.env, config.php 等配置文件不被迁�?
2. **数据库文件保�?*: 数据库连接、迁移脚本等保持私有
3. **备份文件保护**: 所有备份和临时文件不应放入public
4. **访问控制**: 为迁移的目录设置适当�?.htaccess 规则

## 实施注意事项

1. **测试环境验证**: 在测试环境中先执行迁�?
2. **备份重要数据**: 迁移前创建完整备�?
3. **逐步迁移**: 分阶段执行，每次迁移后验证功�?
4. **路径更新**: 迁移后更新所有相关路径引�?
5. **性能监控**: 迁移后监控系统性能和安全�?
...
MARKDOWN;
    ...
    private function generateMigrationScript(): void
    {
        ...
        echo "🚀 迁移执行脚本已生�? execute_comprehensive_migration.php\n";
