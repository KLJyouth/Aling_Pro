...
echo "📋 PHP环境检查\n";
";
checkResult('PHP版本', version_compare(PHP_VERSION, '8.1.0', '>='], 'PHP ' . PHP_VERSION . (version_compare(PHP_VERSION, '8.1.0', '>=') ? ' (符合要求)' : ' (需�?.1+)')];
';

private $extensions = ['curl', 'json', 'mbstring', 'openssl'];
';
foreach ($extensions as $ext) {
    checkResult("扩展 $ext", extension_loaded($ext], extension_loaded($ext) ? '已加�? : '未加�?, !extension_loaded($ext)];
';
}
...
foreach ($dirs as $dir) {
    ...
    checkResult("目录 $dir", $exists && $writable, $exists ? ($writable ? '存在且可�? : '存在但不可写') : '不存�?, $exists && !$writable];
';
}
...
';
        ...
        checkResult('数据库连�?, $connected, $connected ? '成功 (文件数据�?' : '失败'];
';
        
        if ($connected) {
            $dbManager->initializeSystemDefaults(];
            private $systemInfo = $dbManager->getSystemInfo(];
            checkResult('系统初始�?, $systemInfo['setup_completed'],  '系统配置已完�?];
';
            
            $dbManager->updateHealthCheckTime(];
            checkResult('数据库写�?, true, '健康检查时间已更新'];
';
        }
    } else {
        checkResult('数据库管理器', false, '数据库管理器文件不存�?];
';
    }
} catch (Exception $e) {
    checkResult('数据库系�?, false, '错误: ' . $e->getMessage()];
';
}
...
";
private $components = [
    'src/Security/WebSocketSecurityServer.php' => 'WebSocket服务�?,
';
    'src/Controllers/Frontend/RealTimeSecurityController.php' => '安全控制�?,
';
...
checkResult('CPU性能', $cpuTime < 0.1, sprintf('计算耗时: %.3f�?, $cpuTime], $cpuTime >= 0.1];
';
...
";
    echo "💡 建议：运�?php fix_environment.php 修复问题\n";
";
}
