        ...
        // 首页AI聊天系统 - 真实API集成版本
// 创建时间: 2025-06-06
// 版本: 2.0.0 - 替换localStorage为真实数据库API

console.log('🚀 加载首页AI聊天系统模块...');

/**
 * 首页AI聊天系统类 - 真实API集成
 */
class HomepageAIChat {
    constructor() {
        this.isInitialized = false;
        this.chatHistory = [];
        this.isTyping = false;
        this.currentTypingEffect = null;
        this.currentSessionId = null;        this.apiEndpoints = {
            sendMessage: '/api/v1/chat/sessions',
            getSessions: '/api/v1/chat/sessions',
            createSession: '/api/v1/chat/sessions',
            getMessages: '/api/v1/chat/sessions/{id}/messages',
            saveMessage: '/api/v1/chat/messages',
            auth: '/api/v1/auth'
        };
    ...
    async init() {
        ...
        console.log('✅ 首页AI对话系统初始化完成 - 真实API模式');
    }
    ...
    async checkAuthStatus() {
        try {
            ...
            const response = await fetch(`${this.apiEndpoints.auth}/me`, {
                method: 'GET',
                headers: {
            ...
            if (response.ok) {
                ...
                console.log('✅ 用户已认证:', this.currentUser.data?.email || this.currentUser.data?.username);
            } else {
                this.isAuthenticated = false;
                localStorage.removeItem('auth_token');
            }
        } catch (error) {
            console.warn('认证检查失败，使用访客模式:', error);
            this.isAuthenticated = false;
        }
    }
    ...
    hideChatWidget() {
      ...
      async sendMessage() {
        ...
        try {
            ...
            if (response.success) {
                this.addMessage(response.data.response || response.data.content, 'ai');
                // 更新会话信息
                if (response.data.session_id) {
                    this.currentSessionId = response.data.session_id;
                }
            } else {
                this.addMessage('抱歉，发生了错误：' + (response.error || '请稍后再试'), 'ai', true);
            }
            ...
            this.hideTypingIndicator();
            this.addMessage('抱歉，连接服务器失败。请检查网络连接后重试。', 'ai', true);
    ...
    async createNewSession() {
        try {
            ...
            const response = await fetch(this.apiEndpoints.createSession, {
                method: 'POST',
                headers,
                body: JSON.stringify({
                    title: '首页聊天会话',
    ...
    async sendMessageToAPI(message) {
        ...
        const endpoint = this.currentSessionId 
            ? `${this.apiEndpoints.sendMessage}/${this.currentSessionId}/messages`
            : this.apiEndpoints.createSession;

        const body = this.currentSessionId 
        ...
        // 如果是新会话且成功创建，更新会话ID
        if (!this.currentSessionId && data.success && data.data.id) {
            this.currentSessionId = data.data.id;
        }
    ...
    sendQuickMessage(text) {
      ...
      addMessage(content, sender, isError = false) {
        ...
        // 如果用户已认证且有当前会话，保存到数据库
        if (this.isAuthenticated && this.currentSessionId && !isError) {
            this.saveMessageToDatabase(messageObj);
        } else {
            // 保存到本地存储作为备份
    ...
    async saveMessageToDatabase(messageObj) {
        try {
            ...
            const response = await fetch(this.apiEndpoints.saveMessage, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`
                ...
                body: JSON.stringify({
                    ...
                    role: messageObj.sender === 'user' ? 'user' : 'assistant',
            ...
            if (!response.ok) {
                console.warn('Failed to save message to database, using localStorage fallback');
                this.saveChatToLocalStorage();
            }
        } catch (error) {
    ...
    async saveChatToServer() {
        try {
            ...
            const response = await fetch('/api/v1/chat/conversations', {
                ...
                body: JSON.stringify({
                    title: `首页聊天 - ${new Date().toLocaleString()}`,
    ...
    saveChatToLocalStorage() {
        try {
            localStorage.setItem('homepage-chat-history', JSON.stringify(this.chatHistory));
    ...
    async loadChatHistoryFromAPI() {
        try {
            ...
            const response = await fetch(this.apiEndpoints.getSessions, {
                method: 'GET',
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json'
            ...
            if (data.success && data.data && data.data.length > 0) {
                // 获取最近的会话
                const recentSession = data.data[0];
                this.currentSessionId = recentSession.id;
                
                // 加载该会话的消息
                await this.loadSessionMessages(recentSession.id);
                
                console.log('✅ 从数据库加载聊天历史');
            }
        } catch (error) {
    ...
    async loadSessionMessages(sessionId) {
        try {
            ...
            const response = await fetch(
                this.apiEndpoints.getMessages.replace('{id}', sessionId),
                {
                    method: 'GET',
                    headers: {
                        'Authorization': `Bearer ${token}`,
            ...
            if (data.success && data.data) {
                // 显示最近的5条消息
                const recentMessages = data.data.slice(-5);
                recentMessages.forEach(msg => {
                    this.addMessage(msg.content, msg.role === 'user' ? 'user' : 'ai', false, false);
                });
            }
        } catch (error) {
     ...
     * 加载聊天历史（智能加载：API优先，本地存储回退）
     */
    async loadChatHistoryFromStorage() {
        try {
            const isAuthenticated = await this.checkAuthentication();
            
            if (isAuthenticated) {
                // 认证用户：尝试从API加载
                const loaded = await this.loadChatHistoryFromServer();
                if (loaded) return;
            }
    ...
    async loadChatHistoryFromServer() {
        try {
            const response = await fetch('/api/v1/chat/conversations?source=homepage&limit=1', {
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json',
                    'X-Requested-With': 'XMLHttpRequest'
    ...
    loadChatHistoryFromLocalStorage() {
        try {
            ...
            if (saved) {
                ...
                console.log('📱 从本地存储加载临时聊天历史');
            }
        } catch (error) {
            console.warn('从localStorage加载聊天历史失败:', error);
        }
    }
    ...
    async saveChatHistory() {
      ...
      async clearHistory() {
        ...
        if (chatMessages) {
            ...
            chatMessages.innerHTML = `
                <div class="flex items-start space-x-2">
                    ...
                    <div class="flex-1">
                        <p class="text-sm text-gray-600">您好！我是珑凌科技的AI智能助手。我可以帮助您了解我们的产品和服务、回答技术问题、提供解决方案建议等。请问有什么可以为您效劳的吗？</p>
                    </div>
                </div>
            `;
        }
        ...
        try {
            ...
            if (isAuthenticated) {
                // 认证用户：清除服务器端数据
                await this.clearHistoryFromServer();
            }
            
            // 清除本地存储（无论认证状态）
            localStorage.removeItem('homepage-chat-history');
            localStorage.removeItem('chatHistory'); // 清理旧的存储键
            
            console.log('✅ 聊天历史已清空');
        } catch (error) {
            console.warn('清空聊天历史时出错:', error);
            // 确保本地存储被清理
            localStorage.removeItem('homepage-chat-history');
    ...
    // 从服务器清除聊天历史
    async clearHistoryFromServer() {
        try {
            const response = await fetch('/api/v1/chat/conversations?source=homepage', {
                method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json',
                    'X-Requested-With': 'XMLHttpRequest'
