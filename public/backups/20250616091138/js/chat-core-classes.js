// 聊天核心类定义 - 修复缺失的类和全局对象
// 创建时间: 2025-05-30
// 更新: 2025-06-06 - 集成真实API功能

console.log('🔧 加载聊天核心类模块...');

// MessageProcessor 类 - 处理消息（真实API集成版本）
class MessageProcessor {
    constructor() {
        ...
        console.log('📨 MessageProcessor 初始化完成 - 真实API模式');
    }
    ...
    async processMessage(message, type = 'text', sessionId = null) {
        try {
            ...
            // 保存到数据库（如果有会话ID）
            if (sessionId) {
                await this.saveMessageToAPI(processedMessage);
            }
     ...
     * 保存消息到API
     */
    async saveMessageToAPI(message) {
        try {
            ...
            if (!response.ok) {
                console.warn('Failed to save message to API');
                return false;
            }
     ...
     * 从API加载消息历史
     */
    async loadMessagesFromAPI(sessionId, limit = 50) {
        try {
            const token = localStorage.getItem('auth_token');
            if (!token) return [];

            const url = this.apiEndpoints.getMessages.replace('{id}', sessionId) + `?limit=${limit}`;
            ...
            if (!response.ok) {
                console.warn('Failed to load messages from API');
                return [];
            }
