...
class CppCodeAnimation {
    ...
    async startAnimation() {
        ...
        try {
            ...
            // 代码爆炸动画（已包含全屏3D Hello World效果）
            await this.explodeCode();
            
            if (this.isDestroyed) return;
            
            // 注释掉原来的Hello World显示，现在在explodeCode中处理
            // await this.showHelloWorld();
            
            if (this.isDestroyed) return;
            
            // 注释掉收缩，因为新动画流程不需要
            // await this.shrinkToSphere();
            ...
            if (style.parentNode) {
                style.parentNode.removeChild(style);
                    ...
                    span.style.cssText = `
                        ...
                        animation-delay: ${charIndex * 0.02}s;
                    `;
                    line.appendChild(span);
                });
            }, index * 50);
    ...
    async showHelloWorld() {
        this.helloWorldText.style.display = 'block';
        await this.delay(2000);
    }
    
    async shrinkToSphere() {
        this.helloWorldText.classList.add('shrinking');
        await this.delay(1500);
        this.helloWorldText.style.display = 'none';
    }
    
    show3DSphere() {
        this.sphereContainer.classList.add('active');
    }
            ...
            if (this.sphere && this.renderer && this.scene && this.camera) {
                try {
                    if (this.sphere.material.uniforms) {
                        // 更新时间uniform
                        this.sphere.material.uniforms.time.value = time * 0.001;
                    }
                    
                    // 球体旋转
                    const rotationSpeed = this.performanceMode === 'high' ? 1 : 0.5;
                    this.sphere.rotation.x += 0.01 * rotationSpeed;
                    this.sphere.rotation.y += 0.015 * rotationSpeed;
                    this.sphere.rotation.z += 0.005 * rotationSpeed;
    ...
    reset() {
        ...
        if (this.helloWorldText) {
            this.helloWorldText.style.display = 'none';
            this.helloWorldText.classList.remove('shrinking');
        }
        
        if (this.sphereContainer) {
            this.sphereContainer.classList.remove('active');
        }
    }
...
// 全局初始化函数
window.initCppAnimation = function(containerId) {
    return new CppCodeAnimation(containerId);
};
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (container) {
        ...
        restartBtn.style.cssText = `
            ...
            z-index: 10;
        `;
        restartBtn.onclick = () => window.cppAnimation.restart();
