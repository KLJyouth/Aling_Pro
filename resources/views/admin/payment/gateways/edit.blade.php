                    ...
                    <form action="{{ route("admin.payment.gateways.update", $gateway->id) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                    <div class="custom-control custom-switch">
                                        <input type="checkbox" class="custom-control-input" id="is_active" name="is_active" value="1" {{ old("is_active", $gateway->is_active) ? "checked" : "" }}>
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    <div class="custom-control custom-switch">
                                        <input type="checkbox" class="custom-control-input" id="is_test_mode" name="is_test_mode" value="1" {{ old("is_test_mode", $gateway->is_test_mode) ? "checked" : "" }}>
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" class="form-control @error("sort_order") is-invalid @enderror" id="sort_order" name="sort_order" value="{{ old("sort_order", $gateway->sort_order) }}">
                            ...
                            @foreach($fields as $field => $label)
                                <div class="form-group">
                                    ...
                                    @if(in_array($field, ["private_key", "key", "secret_key", "client_secret", "cert_password"]))
                                        <input type="password" class="form-control @error("config.$field") is-invalid @enderror" id="config_{{ $field }}" name="config[{{ $field }}]" value="{{ old("config.$field", $config[$field] ?? "") }}" required>
                                    ...
                                    @elseif(in_array($field, ["cert_path", "key_path"]))
                                        ...
                                        @if(isset($config[$field]) && $config[$field])
                                            <small class="form-text text-muted">当前文件: {{ $config[$field] }}</small>
                                        @endif
                                    @elseif($field === "mode" && $gateway->code === "paypal")
                                        <select class="form-control @error("config.$field") is-invalid @enderror" id="config_{{ $field }}" name="config[{{ $field }}]" required>
                                            <option value="sandbox" {{ old("config.$field", $config[$field] ?? "") === "sandbox" ? "selected" : "" }}>沙箱环境 (Sandbox)</option>
                                            <option value="live" {{ old("config.$field", $config[$field] ?? "") === "live" ? "selected" : "" }}>生产环境 (Live)</option>
                                        </select>
                                    @elseif($field === "currency")
                                        <select class="form-control @error("config.$field") is-invalid @enderror" id="config_{{ $field }}" name="config[{{ $field }}]" required>
                                            <option value="CNY" {{ old("config.$field", $config[$field] ?? "") === "CNY" ? "selected" : "" }}>人民币 (CNY)</option>
                                            <option value="USD" {{ old("config.$field", $config[$field] ?? "") === "USD" ? "selected" : "" }}>美元 (USD)</option>
                                            <option value="EUR" {{ old("config.$field", $config[$field] ?? "") === "EUR" ? "selected" : "" }}>欧元 (EUR)</option>
                                            <option value="GBP" {{ old("config.$field", $config[$field] ?? "") === "GBP" ? "selected" : "" }}>英镑 (GBP)</option>
                                            <option value="JPY" {{ old("config.$field", $config[$field] ?? "") === "JPY" ? "selected" : "" }}>日元 (JPY)</option>
                                        </select>
                                    @else
                                        <input type="text" class="form-control @error("config.$field") is-invalid @enderror" id="config_{{ $field }}" name="config[{{ $field }}]" value="{{ old("config.$field", $config[$field] ?? "") }}" required>
                                    @endif
                                    @error("config.$field")
                                        <span class="invalid-feedback">{{ $message }}</span>
                                    @enderror
