    ...
    <div class="row">
        <div class="col-md-12">
            <div class="card">
                ...
                <!-- /.card-header -->
                <!-- form start -->
                <form role="form" method="POST" action="{{ route('admin.management.roles.update', $role->id) }}">
                    @csrf
                    @method('PUT')
                    <div class="card-body">
                        <div class="form-group">
                            <label for="name">角色名称 <span class="text-danger">*</span></label>
                            <input type="text" class="form-control @error('name') is-invalid @enderror" id="name" name="name" placeholder="请输入角色名称" value="{{ old('name', $role->name) }}" required>
                            @error('name')
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control @error('slug') is-invalid @enderror" id="slug" name="slug" placeholder="请输入角色标识（英文字母、数字和下划线）" value="{{ old('slug', $role->slug) }}" {{ $role->is_protected ? 'readonly' : '' }} required>
                            @error('slug')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                            ...
                            <small class="form-text text-warning">此角色为系统保护角色，标识不可修改</small>
                            @endif
                        </div>
                        <div class="form-group">
                            <label for="description">角色描述</label>
                            <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description" rows="3" placeholder="请输入角色描述">{{ old('description', $role->description) }}</textarea>
                            @error('description')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                        ...
                        <div class="form-group">
                                    ...
                                    <div class="card">
                                        ...
                                        <div class="card-body">
                                            <div class="row">
                                                ...
                                                <div class="col-md-3">
                                                    <div class="form-check">
                                                        <input class="form-check-input permission-checkbox group-{{ $group->id }}" type="checkbox" id="permission-{{ $permission->id }}" name="permissions[]" value="{{ $permission->id }}" {{ in_array($permission->id, old('permissions', $rolePermissions)) ? 'checked' : '' }}>
                            ...
                            <div class="alert alert-warning mt-3">
                                <i class="fas fa-exclamation-triangle"></i> 注意：此角色为系统保护角色，修改权限可能会影响系统功能。
                            </div>
                            @endif
                        </div>
                    </div>
